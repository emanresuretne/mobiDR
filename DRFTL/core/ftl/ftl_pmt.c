...
STATUS PMT_Search(PGADDR page_addr, LOG_BLOCK* block, PAGE_OFF* page) {
  ...
  if (ret == STATUS_SUCCESS) {
    ...
    cluster_addr = PM_NODE_ADDRESS(root_table.page_mapping_nodes[cluster]);   // root_table.page_mapping_nodes[cluster] 0~502
    ASSERT(cluster_addr != 0);

    pm_node = cluster_addr[PAGE_IN_CLUSTER(page_addr)];   // PAGE_IN_CLUSTER(page_addr) 0~511
...
// 07/01/2020 Wen added
// get array of logical block addresses I which have valid mapping

int PMT_isvalid(PGADDR page_addr){
  PM_NODE_ADDR* cluster_addr;
  PM_NODE_ADDR pm_node;
  int ret = 0;
  // static char valid_mapping[250000] = 0+'0';
  PMT_CLUSTER cluster;//计算逻辑I地址所在簇号
  
  // max_page = FTL_Capacity() - 1;

  cluster = CLUSTER_INDEX(page_addr);
  if (PM_NODE_IS_CACHED(root_table.page_mapping_nodes[cluster]) == FALSE) {
    /* load page in cache */
    ret = PMT_Load(PM_NODE_BLOCK(root_table.page_mapping_nodes[cluster]),
                   PM_NODE_PAGE(root_table.page_mapping_nodes[cluster]),
