...
extern PHY_BLOCK current_record_block;
extern PAGE_OFF current_record_page;
extern int max_records;
extern UINT32 record_index;
extern UINT32 record_cache_page[512];
...
STATUS DATA_Write(PGADDR addr, void* buffer, BOOL is_hot) {
  ...
  if (buffer != NULL) {
    ...
    // wen added
    // backup physical page and logical page
    
    if(addr!=240000 && addr!=240010 && addr!=240020 && addr!=240030 && addr!=240040 && addr!=240014 && addr!=240024 && addr!=240034  &&restore_flag==0 && initial_flag ==1){
     // uart_printf("current_record_block=%d, current_record_page=%d\n", current_record_block, current_record_page);
     // uart_printf("current record index = %d\n", record_index);
     // uart_printf("block=%d, page=%d\n", block, page);
      // show_bytes(buffer, 2048);
      if(record_index<511){
        record_cache_page[record_index] = ((((block)<<PAGE_PER_BLOCK_SHIFT)+(page))<<2) + 1;
        record_index += 1;
      } else {
        // meta_data[0] = 0;
        record_cache_page[record_index] = ((((block)<<PAGE_PER_BLOCK_SHIFT)+(page))<<2) + 1;
        record_index = 0;
        if (current_record_page == PAGE_PER_PHY_BLOCK) {
        ...
        ret = UBI_Write(current_record_block, current_record_page,
                        record_cache_page, meta_data[page], FALSE);
...
STATUS Reclaim_block(UINT32 block_num, UINT32 total_valid_page){
  LOG_BLOCK dirty_block;
  PAGE_OFF page;
  LOG_BLOCK true_block = INVALID_BLOCK;
  PAGE_OFF true_page = INVALID_PAGE;
      ...
      for (page = 0; page < PAGE_PER_PHY_BLOCK - 1; page++) {
        ...
        if (ret == STATUS_SUCCESS) {
          if (true_block == dirty_block && true_page == page) {//若是valid页
            ...
            if (ret == STATUS_SUCCESS) {
              /* write buffer */
              ret = FTL_Write(pages_buffer[page][0], data_buffer);
            }
  ...
  } else {       // 会擦除完全未使用的块，对结果应该没影响
    /* erase dirty blocks, update journals, and no need to copy pages */
    ret = UBI_Erase(block_num, block_num);
  }
...
STATUS Threshold_DATA_Reclaim(void) {
  UINT32 i;
  UINT32 total_valid_page = 0;
  STATUS ret = STATUS_SUCCESS;
  ...
  if (ret == STATUS_SUCCESS) {
    for (i = DATA_START_BLOCK; i <= DATA_LAST_BLOCK; i++) {
      if (i == PM_NODE_BLOCK(root_table.hot_journal[0]) || i == PM_NODE_BLOCK(root_table.cold_journal[0])){
        continue;
      }
      if (block_dirty_table[i] >= threshold){
        total_valid_page = MAX_DIRTY_PAGES - block_dirty_table[i];
        Reclaim_block(i, total_valid_page);
      }
    }
  }
