...
// --------------------------- //
	...
	char server_addr[255] = "192.168.1.26";
...
// --------------------------- //
  ...
  if (posix_memalign((void *)&buf, SECTOR, BUF_SIZE)) {
      ...
      if ((fd = open("/dev/sda", O_WRONLY | O_TRUNC | O_DIRECT)) == -1) perror("[open]");
      ...
      op.paramTypes = TEEC_PARAM_TYPES(TEEC_MEMREF_TEMP_INPUT,
              TEEC_VALUE_OUTPUT, TEEC_MEMREF_TEMP_INPUT, TEEC_VALUE_OUTPUT);
      ...
      strcpy(filename, "/home/pi/Desktop/flash.back");
      strcat(filename, argv[1]);

      if ((fd = open("/dev/sda", O_RDONLY | O_DIRECT)) == -1) perror("[open]");
      if ((fd2 = open(filename, O_WRONLY | O_CREAT | O_APPEND | O_TRUNC, 755)) == -1) perror("[open]");

      /* buf size , buf alignment and offset has to observe hardware restrictions */
      
      do 
      {
          lseek(fd, 960000*512, SEEK_SET);
          ...
          if((nr = read(fd, buf, BUF_SIZE)) == -1) {
              ...
              printf("dtime is %ld\n", dtime);
              if (strcmp(back_up, buf) != 0) {
                  if((nw = write(fd2, buf, BUF_SIZE)) == -1) perror("[pwrite]");
              }
              lseek(fd, 960000*512, SEEK_SET);
          }
      } while(strcmp(back_up, buf) != 0);
    ...
    strncpy(buf3, tmp1, 2059);
		...
		while(string_cmp(buf, back_up) != 0)
		{
        ...
        /*
				printf("page read: \n");
				show_bytes(buf3, sizeof(buf3));
        printf("hmac: \n");
				show_bytes(buf2, sizeof(buf2));
        hmac_sha1(digest2, buf3, 2060);
        printf("verify result is %d:\n", string_cmp(digest2, buf2));
        */
...
// --------------------------- //
	...
	op.paramTypes = TEEC_PARAM_TYPES(TEEC_MEMREF_TEMP_OUTPUT,
					  TEEC_NONE, TEEC_NONE, TEEC_VALUE_OUTPUT);
